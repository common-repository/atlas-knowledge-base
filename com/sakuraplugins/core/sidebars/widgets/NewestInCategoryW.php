...
class ATLAS_NewestInCategoryW extends WP_Widget {

	const DEFAULT_POST_NO = 5;

	public function __construct(){
		parent::__construct(
			'atlas_newest_in_category',
			esc_html__('Atlas: Newest in category', 'atlas-knb-textdomain'),
			array(
				'classname' => 'atlas_widget',
				'description' => esc_html__('Shows newest in category. Note! This widget only works within Atlas - Category sidebar', 'atlas-knb-textdomain')
			)
		);
	}

	public function form($instance) {
		$title = isset($instance['title' ]) ? $instance['title'] : esc_html__('Newest in category', 'atlas-knb-textdomain');
