...
class ATLAS_RecentlyUpdatedW extends WP_Widget {

	const DEFAULT_POST_NO = 5;

	public function __construct(){
		parent::__construct(
			'atlas_recently_updated',
			esc_html__('Atlas: Recently updated', 'atlas-knb-textdomain'),
			array(
				'classname' => 'atlas_widget',
				'description' => esc_html__('Shows recently updated articles', 'atlas-knb-textdomain')
			)
		);
	}

	public function form($instance) {
		$title = isset($instance['title' ]) ? $instance['title'] : esc_html__('Recently updated', 'atlas-knb-textdomain');
